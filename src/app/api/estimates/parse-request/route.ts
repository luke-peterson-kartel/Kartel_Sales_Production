...
// System prompt for parsing client requests
  ...
  try {
    ...
    if (!clientRequest || clientRequest.trim().length < 10) {
      return NextResponse.json(
        { error: 'Please provide a client request (at least 10 characters)' },
        { status: 400 }
      );
    }
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [
        {
          role: 'user',
          content: `Parse the following client request and extract all deliverables. If the format is unusual (CSV, table, email, etc.), do your best to interpret it.
...
Remember to return valid JSON with the exact structure specified.`,
    ...
    console.error('Error parsing client request:', error);
    return NextResponse.json(
      { error: 'Failed to parse client request' },
      { status: 500 }
    );
  }
}
